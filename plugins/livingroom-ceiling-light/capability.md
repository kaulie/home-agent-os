...
用 **home-server 本机 TTS** 对智能音箱喊唤醒词，再发开/关指令。不经过 LLM，不录音。

**Brain 不执行**；只通过心跳看到 `light.set`，再把 plan 派到具备该能力的 Edge（home-server）。
...
| 执行方 | Mac Edge home-server（`mac_edge.plugins.livingroom_light`） |
...
无法验证灯是否真亮：home-server 必须在客厅、扬声器开着、音量够「小书」听见。
...
```json
{
  ...
  "step": 1,
  "assigned_edge_id": "<home-server-edge-id>",
  "input_constrict": {
