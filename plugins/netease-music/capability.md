...
| capability | 能 | 不能 |
|------------|----|------|
| `music.play` | 按歌名播放（可选作者限定）；「xxx的歌」或仅 `artist` 时清队列连播同艺人最多 20 首；**无 song/artist 时先继续播放，否则每日推荐连播** | 无歌名只点歌手（非「xxx的歌」句式）用 query.content；下载/缓存索引 |
...
- **歌手连播**：`song` 形如「xxx的歌/歌曲」或仅有 `artist` 时，搜该艺人最多 20 首 → `queue clear` → `play` 第一首 → `queue add` 其余；明确歌名（如「十年」「陈奕迅的十年」）仍单次 `play --song`，不清队列。
- **句首数量词**：`几首`/`几曲`/`一些`/`N首`（阿拉伯数字）不是歌手名的一部分。剥掉后再解析「xxx的歌」；`几首` 类默认连播 5 首，`N首` 按 N（1–20）截断。可选入参 `count` 覆盖上限。
- **空参开播**：`song` 与 `artist` 皆空（如用户只说「播放音乐」）→ 先 `ncm-cli resume`；失败则 `recommend daily`（最多 20 首）走与歌手连播相同的清队列 + play + queue add。不依赖 `state`（云音乐模式下不可用）。
