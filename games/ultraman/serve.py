#!/usr/bin/env python3
"""LAN host for the Ultraman punch game. Chromecast must use the printed LAN URL.

  python3 games/ultraman/serve.py

Page polls GET /events every 5s. Switch to the live stream later with ?stream=1.

Inject a punch from the pose parser:

  curl -s -X POST http://<lan-ip>:8099/punch
  curl -s -X POST http://<lan-ip>:8099/punch -H 'Content-Type: application/json' \\
    -d '{"type":"punch","ts":123}'
"""
...
PORT = int((os.environ.get("ULTRAMAN_GAME_PORT") or "8099").strip() or "8099")
_CLIENTS: list[Queue] = []
_EVENTS: list[dict] = []
_NEXT_ID = 1
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if path not in ("/punch", "/punch/", "/action", "/action/"):
            self.send_response(404)
            self.end_headers()
            return
        length = int(self.headers.get("Content-Length") or 0)
        raw = self.rfile.read(length) if length else b""
        event: dict = {"type": "punch", "ts": int(time.time() * 1000)}
        if raw.strip():
            try:
...
def main() -> None:
    ...
    print("  events GET  http://%s:%s/events?since=<id>   (page polls every 5s)" % (ip, PORT))
    print("  stream GET  http://%s:%s/events/stream       (later: ?stream=1)" % (ip, PORT))
    print("  punch  POST http://%s:%s/punch" % (ip, PORT))
