...
- 拉取 URL：`BuildConfig.DEFAULT_COMMANDS_PULL_URL`  
  `http://brain.local:9527/api/v1/devices/living-room/intents?intent_status=intent_parsed`
...
默认服务器：`http://brain.local:9527`（`BuildConfig.DEFAULT_BRAIN_BASE_URL`；HTTP 须先解析 IPv4）。
