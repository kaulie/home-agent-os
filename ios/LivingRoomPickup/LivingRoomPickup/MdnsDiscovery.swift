...
enum MdnsDiscovery {
    ...
    static func logLaunchNetworkContext() {
        ...
        dlog(
            "local network permission: not directly readable; if browse error=-65570 or 0 results, check Settings → 拾音 → Local Network",
            category: "launch"
        )
    }
    ...
    private static func probeServiceBatchSync(
        ...
        batchIndex: Int,
        serviceLabel: String,
        probe: (_ host: String, _ port: Int, _ timeout: TimeInterval, _ completion: @escaping (Bool, String) -> Void) -> Void,
        ...
        for hostOctet in octets {
            ...
            probe(ip, port, timeout) { ok, detail in
                lock.lock()
                results.append((ip, ok))
                if ok {
                    if found == nil { found = ip }
                    dlog("\(serviceLabel) probe HIT \(ip):\(port) \(detail)", category: "LAN scan")
                }
                lock.unlock()
                group.leave()
            }
            ...
            for subnet in subnets {
                ...
                while idx < octets.count, Date() < deadline {
                    ...
                    probeServiceBatchSync(
                        ...
                        batchIndex: batches,
                        serviceLabel: "brain",
                        probe: { host, port, timeout, done in
                            probeBrainDetailed(host: host, port: port, timeout: timeout, completion: done)
                        }
                    ) { ip in
                        foundIP = ip
                        sem.signal()
        ...
        static func preferredSubnets() -> [Subnet] {
            ...
            while let current = cursor {
                ...
                if name == "en0" || name.hasPrefix("en") {
                    result.insert(Subnet(prefix: prefix, hostOctet: last, ifaceName: name), at: 0)
                } else {
                    result.append(Subnet(prefix: prefix, hostOctet: last, ifaceName: name))
                }
            }
            return result
        }
    ...
    /// TCP/HTTP must use RFC1918 IPv4. `.local` would be DNS/mDNS, not this policy.
    static func refuseNonIPv4TCP(_ raw: String) -> String? {
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if trimmed.isEmpty {
            return "没有可用的局域网 IPv4，不会用 gateway.local 建 TCP。请先自动发现。"
        }
        let lower = trimmed.lowercased()
        if lower.hasSuffix(".local") || lower.contains(".local.") {
            return "拒绝用 mDNS 名建 TCP（\(trimmed)）。必须先发现 IPv4。"
        }
        if !isUsableLanIPv4(trimmed) {
            return "TCP 目标不是局域网 IPv4（\(trimmed)）"
        }
        return nil
    }
    ...
    static func pickVerifiedGateway(
        ...
        dlog("gateway verify: \(ranked.count) ranked → \(ranked.prefix(maxProbes).map { "\(describeEndpoint($0)) trustworthy=\(isTrustworthyGateway($0))" }.joined(separator: " | "))", category: "ping verify")
        var verified: [Endpoint] = []
        let lock = NSLock()
        let group = DispatchGroup()
        for ep in ranked.prefix(maxProbes) {
            group.enter()
            probeGatewayDetailed(host: ep.host, port: ep.port, timeout: probeTimeout) { ok, detail in
                MdnsDiscovery.dlog("gateway probe GET http://\(ep.host):\(ep.port)/api/v1/video-live/status → \(ok ? "PASS" : "FAIL") \(detail)", category: "ping verify")
                if ok {
                    lock.lock()
                    verified.append(ep)
                    lock.unlock()
    ...
    static func discoverGatewayByLANScan(budget: TimeInterval = 5.0, completion: @escaping (Endpoint?) -> Void) {
        ...
        DispatchQueue.global(qos: .utility).async {
            ...
            for subnet in subnets {
                if Date() >= deadline { break }
                let octets = lanScanHostOctets(for: subnet)
                dlog("gateway probing \(subnet.prefix).x (\(octets.count) hosts, batches of \(batchSize))", category: "LAN scan")
                var idx = 0
                var batches = 0
                while idx < octets.count, Date() < deadline {
                    let end = min(idx + batchSize, octets.count)
                    ...
                    probeServiceBatchSync(
                        ...
                        probe: { host, port, timeout, done in
                            probeGatewayDetailed(host: host, port: port, timeout: timeout, completion: done)
                        }
                    ) { ip in
                        foundIP = ip
                        sem.signal()
    ...
    static func resolveGatewayForAutoDiscover(mdnsTimeout: TimeInterval = 8, completion: @escaping (Endpoint?) -> Void) {
        ...
        group.notify(queue: .global(qos: .userInitiated)) {
            let mdns = candidates.filter { $0.serviceName != "LAN scan" && isUsableLanIPv4($0.host) }
            let scan = candidates.first { $0.serviceName == "LAN scan" && isUsableLanIPv4($0.host) }
            func finish(_ ep: Endpoint?) {
                if let ep {
                    dlog("gateway auto-discover chosen \(describeEndpoint(ep))", category: "mDNS browse")
                } else {
                    dlog("gateway auto-discover: no IPv4 to use", category: "mDNS browse")
                }
                DispatchQueue.main.async { completion(ep) }
            }
            // LAN scan already probed video-live/status. Do not drop a HIT by re-probing.
            if mdns.isEmpty {
                finish(scan)
                return
            }
            pickVerifiedGateway(from: mdns) { best in
                if let best {
                    finish(best)
                } else {
                    dlog(
                        "gateway mDNS verify failed; using LAN scan \(scan?.host ?? "nil")",
    ...
    static func probeGateway(host: String, port: Int = 8790, timeout: TimeInterval = 1.0, completion: @escaping (Bool) -> Void) {
        probeGatewayDetailed(host: host, port: port, timeout: timeout) { ok, _ in
            completion(ok)
        }
    }

    private static func probeGatewayDetailed(
        host: String,
        port: Int = 8790,
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            guard let data,
                ...
                completion(false, "HTTP \(http.statusCode) invalid JSON")
                return
            }
            if json["status"] != nil {
                completion(true, "HTTP \(http.statusCode) status OK")
            } else {
                completion(false, "HTTP \(http.statusCode) missing status field")
            }
        }.resume()
    }
    ...
    static func pickVerifiedBrain(
        ...
        for ep in ranked.prefix(maxProbes) {
            group.enter()
            probeBrainDetailed(host: ep.host, port: ep.port, timeout: probeTimeout) { ok, detail in
                MdnsDiscovery.dlog("brain probe GET http://\(ep.host):\(ep.port)/api/v1/ping → \(ok ? "PASS" : "FAIL") \(detail)", category: "ping verify")
                if ok {
                    lock.lock()
                    verified.append(ep)
                    lock.unlock()
    ...
    static func probeBrain(host: String, port: Int, timeout: TimeInterval = 1.5, completion: @escaping (Bool) -> Void) {
        probeBrainDetailed(host: host, port: port, timeout: timeout) { ok, _ in
            completion(ok)
        }
    }

    private static func probeBrainDetailed(
        host: String,
        port: Int,
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if json["app"] as? String == "brain" {
                completion(true, "HTTP \(http.statusCode) app=brain")
            } else if json["ok"] as? Bool == true, json["server_time_ms"] != nil {
                completion(true, "HTTP \(http.statusCode) ok=true")
            } else {
                completion(false, "HTTP \(http.statusCode) unexpected JSON")
            }
        }.resume()
    }
    ...
    fileprivate final class SingleServiceResolver: NSObject, NetServiceDelegate {
        ...
        func netServiceDidResolveAddress(_ sender: NetService) {
            ...
            MdnsDiscovery.dlog(
                "resolve OK name=\(sender.name) ips=[\(ipv4)] port=\(sender.port)",
                category: "mDNS resolve"
            )
            finish(ep)
        }
