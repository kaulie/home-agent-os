...
/// MPEG-TS over TCP (callback-based, iOS 12).
final class MpegTsTcpTransport {
    private var connection: NWConnection?
    private let queue = DispatchQueue(label: "legacy.video.live.tcp")
    ...
    func connect(host: String, port: UInt16, completion: @escaping (Error?) -> Void) {
        ...
        conn.stateUpdateHandler = { state in
            switch state {
            case .ready:
                once.finish { DispatchQueue.main.async { completion(nil) } }
            case .failed(let err):
                once.finish {
                    DispatchQueue.main.async {
                        completion(MpegTsTcpTransportError.failed("连接 Mac 失败：\(err.localizedDescription)"))
                    }
                }
            case .cancelled:
                once.finish {
                    DispatchQueue.main.async {
                        completion(MpegTsTcpTransportError.failed("连接已取消。"))
                    }
                }
            default:
                break
