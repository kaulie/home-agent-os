...
- Brain 两个地址槽：LAN 身份 `http://brain.local:9527`（HTTP 用 mDNS 解析出的 IPv4），Cloud 默认 `http://115.190.153.53:9527`。节点页和设置里用 **自动 / 局域网 / 云** 切换；自动时家庭局域网且 `/api/v1/ping` 通 LAN 则走 LAN，否则走 Cloud。强制局域网或云会锁定对应槽，不跟探测走。当前在用地址在「节点」页看。
