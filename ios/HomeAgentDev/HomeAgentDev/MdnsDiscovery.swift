...
enum MdnsDiscovery {
    ...
    @available(iOS 13.0, *)
    static func pickVerifiedBrain(from endpoints: [Endpoint], maxProbes: Int = 3, probeTimeout: TimeInterval = 1.0) async -> Endpoint? {
        guard !endpoints.isEmpty else { return nil }
        let ranked = endpoints.sorted { scoreBrain($0) > scoreBrain($1) }
        for ep in ranked.prefix(maxProbes) {
            if await probeBrain(host: ep.host, port: ep.port, timeout: probeTimeout) {
                return ep
            }
        }
        return ranked.first
