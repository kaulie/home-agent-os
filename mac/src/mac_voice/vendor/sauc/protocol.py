...
# Soft logger — no default FileHandler (mac_voice owns logging).
logger = logging.getLogger("mac_voice.vendor.sauc")
...
class CommonUtils:
    ...
    def convert_wav_with_path(audio_path: str, sample_rate: int = DEFAULT_SAMPLE_RATE) -> bytes:
        try:
            ...
            # Do not delete the caller's source file.
...
class AsrWsClient:
    ...
    async def send_messages(self, segment_size: int, content: bytes) -> AsyncGenerator[None, None]:
        ...
        for i, segment in enumerate(audio_segments):
            ...
            # Chunk size still follows segment_duration; do not pace as live mic.
            # Sleeping 200ms/segment made an 8s clip take ~8s of STT and flooded
            # the listen queue so wake words were dropped as "oldest".
    ...
    async def execute(self, file_path: str,config,payload) -> AsyncGenerator[AsrResponse, None]:
        ...
        try:
            ...
            if CommonUtils.judge_wav(content):
                channels, samp_width, rate, _nframes, pcm = CommonUtils.read_wav_info(content)
                size_per_sec = channels * samp_width * rate
                segment_size = max(1, size_per_sec * self.segment_duration // 1000)
                content = pcm
            else:
